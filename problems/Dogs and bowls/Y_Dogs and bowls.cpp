...
class AVL
{
	...
	void insert(Dog data)
	{
		...
		{
			while(true){
				if(node->data.position <= current->data.position)
				{
	...
	void print()
	{
		...
		while(current != NULL){
			cout<<current->data.position<<" : "<<current->data.busy<<" at "<<current->height<<endl;
			current = this->next_max(current);
		}
	}
	Node* find_best_fit(Node* node, int position, int value,bool flag)
	{
		Node * current = node;
		while(current != NULL)
		{
			if(position >= current->data.position)
			{
				if(!(current->data.busy)&&(position - current->data.position)<(value+1))
				{
					Node * temp = flag ? this->find_best_fit(current->right, position, value, false) : NULL;
					if(temp == NULL)return current;
					if((position - current->data.position) < (temp->data.position - position)){
						return current;
					}else{
						return temp;
					}
					//cout<<endl<<":"<<current->data.position<<":"<<current->data.busy<<" , "<<position<<":"<<value<<endl;
					return current;
				}
				current = current->right;
			}else{
				if(!(current->data.busy)&&(current->data.position - position)<(value+1))
				{
					Node * temp = flag ? this->find_best_fit(current->left, position, value, false) : NULL;
					if(temp == NULL)return current;
					if((current->data.position - position) < (position - temp->data.position)){
						return current;
					}else{
						return temp;
					}
...
void bubbleSort(Bowl* arr, int n)
{
   int i, j;
   for (i = 0; i < n-1; i++)      
...
int main()
{
	...
	for (int i = 0; i < m; ++i)
	{
		//cout<<bowls[i].position<<"outside:"<<bowls[i].value<<endl;
		Node * x = dogsTree.find_best_fit(dogsTree.root, bowls[i].position, bowls[i].value, true);
		if(x != NULL)
		{
			x->data.busy = true;
			num_success++;
